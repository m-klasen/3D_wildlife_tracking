...
def main(args):
	...
	dataset_paths = sorted(os.listdir(args.root_dir)) ##['20201217170333']
	"""
	Lindenthal3D Class 
	Attributes: 
	...
	for ds_pth in dataset_paths:
		dataset = Lindenthal3D(root_dir=args.root_dir, 
							...
							pcl_path="pointcloud_dpt",
							fg_path=f"{ds_pth}/fg_predictions.json",
							inst_segm_path=f"{ds_pth}/detr_instance_predictions_conf_0.9.json", # f"{ds_pth}/dmrcnn_instance_predictions_conf_0.9.json",
							iou3d=args.iou,
							save_eval=True)    
		if args.visualize:
...
if __name__ == "__main__":
	...
	parser.add_argument(
		"--root_dir", type=str, default=f'/media/mlk/storage/AMMOD/ammod_realsense/data/2d/valid', help="path to directory containing video clips",)
	parser.add_argument(
