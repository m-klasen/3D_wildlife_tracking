...
def main(args):
	...
	for ds_pth in dataset_paths:
		...
		pcd_dir = str(Path(dataset_path) / "pointcloud_dpt")
		pcd_files = [f for f in sorted(glob.glob(pcd_dir+"/*"))]
  
		with open(f"{dataset_path}/instances_default.json", "r") as f: 
			annos = json.load(f)
				...
				for inst,track_id in zip(masks,ids):
					...
					if np.array(pcl_processed.points).shape[0] > 20:
						...
						str_to_srite = '%d %d %s 0 0 %f %f %f %f %f %f %f %f %f %f %f %f\n' % (
							i,track_id,'goat',0.,bbox[0],bbox[1],bbox[0]+bbox[2],bbox[1]+bbox[3],size[0],size[1],size[2],center[0],center[1],center[2],0.)
