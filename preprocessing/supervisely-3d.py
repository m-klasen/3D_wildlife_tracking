		...
		for i,(pcd_file, mask_file) in enumerate(zip(pcd_files,masks_files)):
			...
			for inst,track_id in zip(masks,ids):
				...
				if np.array(pcl_processed.points).shape[0] > 20:
					...
					instance = {
						"category": "goat",
                        "gemetryType": "cuboid_3d",
						"geometry": {
							"location": dict(x=center[0],y=center[1],z=center[2]),
							"dimension": dict(length=size[0],width=size[1],height=size[2]),
							"orientation": dict(rotationYaw=0,rotationPitch=0,rotationRoll=270)
						}
					}
					annos_out["labels"].append(instance)
					ann_id+=0
			print(annos_out)
...
if __name__ == "__main__":
	...
	parser.add_argument(
		"--root_dir", type=str , default="/media/mlk/storage/AMMOD/ammod_realsense/data/train", help="path to directory containing video clips"
	)
